...
const ProductAdminPage:FC<Props> = ({ product }) => {
    ...
    const onchangeSize = (size: string) => {
        const currenSizes = getValues('sizes')
        if(currenSizes.includes(size)){
            return setValue('sizes', currenSizes.filter(s => s !== size), {shouldValidate:true})
        }

        setValue('sizes', [...currenSizes, size], {shouldValidate:true} )
    }
    ...
    const onSubmit = async(form: FormData) => {
        if (form.images.length < 2) {
            return alert('Minino 2 imagenes')
        }
        setIsSaving(true)
