...
const createOrder = async (req: NextApiRequest, res: NextApiResponse<Data>)  => {
    ...
    if(!session) {
        return  res.status(401).json({ message: 'Debe estar autenticado par ahacer esto' }) 
    }
    ...
    const dbProducs = await Product.find({_id: {$in: productIds}})

    try {
        const subTotal = orderItems.reduce( ( prev, current ) => {
            const currentPrice = dbProducs.find(prod => prod.id === current._id)?.price;

            if(!currentPrice) {
                throw new Error('Verifique el carrito nuevamente, producto no encontrado')
            }
           return (currentPrice * current.quantity) + prev
        }, 0 );

        const taxRate =  Number(process.env.NEXT_PUBLIC_TAX_RATE || 0);
        const backendTotla = subTotal * (taxRate + 1);

        if(total !== backendTotla ) {
